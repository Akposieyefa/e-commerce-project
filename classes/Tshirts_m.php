...
class Tshirts_m 
{
	...
	{
        $subcatid = $this->getSubCategoryID([$this->slug]);

        if ($subcatid !='') {
            $catid = $this->getCategoryID([$this->slug]);

            $stmt = $this->pdo->preparedStatement("SELECT * FROM `products` WHERE `categoryID`='$catid->categoryID' OR `subCategoryID`='$subcatid->subCategoryID' ORDER BY RAND ()  LIMIT ?,?", $args);
